...
#'
#' @export
tmOutput <- function(outputId, width = "100%", height = 500) {
  htmlwidgets::shinyWidgetOutput(outputId, "d3v5_treemap", width, height,
                                 package = "d3tm")
}

#' @rdname ztmShiny
#' @export
renderTm <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) {
    expr <- substitute(expr)
  } # force quoted
  htmlwidgets::shinyRenderWidget(expr, tmOutput, env, quoted = TRUE)
}
